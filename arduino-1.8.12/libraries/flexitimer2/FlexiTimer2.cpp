...
void FlexiTimer2::set(unsigned long t, double resolution, void (*f)())
{
	float prescaler = 0.0;
	time_units = t ? t : 1;
	func = f;
...
void FlexiTimer2::_overflow()
{
	...
	{
		overflowing = 1;			//防止在定时程序运行期间多次触发
		count = count - time_units; // 减去 time_uints 来弥补错过的触发，不想要可以改成 0
		(*func)();					//执行定时程序
		overflowing = 0;
	}
}
...
#endif
	FlexiTimer2::_overflow(); // 到时间啦，该执行定时程序了
}
